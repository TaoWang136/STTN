...
sys.path.append('../../')
from sttn_prediction.dataloader.data_precess import load_data
from sttn_prediction.models.model import STTN
from sttn_prediction.utils.lr_scheduler import LR_Scheduler
from sttn_prediction.utils.parser import getparse
from sttn_prediction.utils.metrics import getmetrics
...
opt.model_filename = '{}/-proximal={}-period={}-spatial={}-mode={}-pr={}-s={}-FS={}-model_N={}-scptmodel_d={}-{}-{}-{}'.format(
                    opt.save_dir, opt.proximal_size,opt.period_size,opt.spatial,opt.mode,opt.pr,opt.s,opt.FS,opt.model_N,
                    opt.s_model_d,opt.pr_model_d,opt.p_model_d,'64')
...
def train_epoch(data_type,epoch):
    ...
    for idx, (pr, p, target) in enumerate(data):
        ...
        model.zero_grad()# 
        pred = model(pr.float(),opt.mode,opt.pr,opt.s,opt.FS,opt.pr_t,opt.s_t,opt.crash,p.float())
        loss = criterion(pred.float(), target.cuda().float()[:,:,opt.crash])#计算预测值与实际标签之间的梯度
        #print(loss)
...
def predict(test_type='train'):
    ...
    for idx, (pr, p, target) in enumerate(data):
        start = time.time()
        pred = torch.relu(best_model(pr.float(),opt.mode,opt.pr,opt.s,opt.FS,opt.pr_t,opt.s_t,opt.number,p.float()))
        end = time.time()
        t += (end-start)
        predictions.append(pred.float().data.cpu().numpy())
        ground_truth.append(target.float().numpy()[:,:,opt.number])
        loss.append(criterion(pred.float(), target.cuda()[:,:,opt.number]).float().item())
        i += 1
    mrt = t/i
...
if __name__ == '__main__':
    ...
    x_train, y_train, x_test, y_test, mmn = load_data(opt.proximal_size, opt.period_size,opt.test_size,opt.SSM)
    ...
    if os.path.isfile(best_model):
        ...
        model = STTN(opt.proximal_size, external_size, opt.model_N, opt.k, opt.spatial,opt.pr_model_d,opt.s_model_d,opt.p_model_d).cuda()#加载transformer模型，.cuda()使其在GPU中运算。
